const todoList = () => {
  ...
  const overdue = () => {
    ...
    const arr = all.filter((student) => student.dueDate == yesterday);
    return arr;
  };

  const dueToday = () => {
    // Write the date check condition here and return the array of todo items that are due today accordingly.
    const arr = all.filter((student) => student.dueDate === today);
    return arr;
  };

  const dueLater = () => {
    // Write the date check condition here and return the array of todo items that are due later accordingly.
    const arr = all.filter((student) => student.dueDate === tomorrow);
